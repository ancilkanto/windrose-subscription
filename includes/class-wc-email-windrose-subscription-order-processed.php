...
class WC_Email_Windrose_Subscription_Order_Processed extends \WC_Email {
    ...
    private function ensure_email_enabled() {
        ...
        if (!isset($enabled_emails['windrose_subscription_order_processed_enabled']) || $enabled_emails['windrose_subscription_order_processed_enabled'] !== 'yes') {
            $enabled_emails['windrose_subscription_order_processed_enabled'] = 'yes';
            update_option('woocommerce_email_settings', $enabled_emails);
            error_log('Windrose Subscription: Enabled subscription order processed email');
        }
    }

    public function trigger( $subscription_id ) {
        error_log('Windrose Subscription: Order processed email trigger method called for subscription ID: ' . $subscription_id);
        ...
        if (!$subscription) {
            error_log('Windrose Subscription: No subscription found for order processed email ID: ' . $subscription_id);
            return;
        }
        $user = get_userdata($subscription->user_id);
        $this->recipient = $user ? $user->user_email : '';
        $this->object = $subscription;
        error_log('Windrose Subscription: Order processed email recipient: ' . $this->recipient);
        error_log('Windrose Subscription: Order processed email enabled: ' . ($this->is_enabled() ? 'Yes' : 'No'));
        if ( ! $this->is_enabled() || ! $this->get_recipient() ) {
            error_log('Windrose Subscription: Order processed email not sent - disabled or no recipient');
            return;
        }
        error_log('Windrose Subscription: Sending order processed email to: ' . $this->get_recipient());
