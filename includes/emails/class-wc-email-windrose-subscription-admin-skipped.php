...
if (class_exists('WC_Email')) {
    class WC_Email_Windrose_Subscription_Admin_Skipped extends \WC_Email {
    ...
    private function ensure_email_enabled() {
        ...
        if (!isset($enabled_emails[$email_key . '_heading']) || empty($enabled_emails[$email_key . '_heading'])) {
            $enabled_emails[$email_key . '_heading'] = __( 'Subscription order skipped', 'windros-subscription' );
            update_option('woocommerce_email_settings', $enabled_emails);}
        
        if (!isset($enabled_emails[$email_key . '_subject']) || empty($enabled_emails[$email_key . '_subject'])) {
            $enabled_emails[$email_key . '_subject'] = __( 'Subscription order skipped', 'windros-subscription' );
            update_option('woocommerce_email_settings', $enabled_emails);}
    }
    ...
    public function trigger( $subscription_order_id ) {if ( !$subscription_order_id ) {return;
        ...
        global $wpdb;
        $subscription_orders_table = $wpdb->prefix . (defined('WINDROS_SUBSCRIPTION_ORDERS_TABLE') ? WINDROS_SUBSCRIPTION_ORDERS_TABLE : 'windrose_subscription_orders');
        $subscription_order = $wpdb->get_row($wpdb->prepare("SELECT * FROM $subscription_orders_table WHERE id = %d", $subscription_order_id));
        if (!$subscription_order) {return;
        }
        
        // Set admin email as recipient
        $this->recipient = get_option('admin_email');
        $this->object = $subscription_order;
        ...
        // error_log('Windrose Subscription: Admin skipped email send result: ' . ($result ? 'Success' : 'Failed'));
    ...
    public function get_content_html() {
        ...
        wc_get_template(
            $this->template_html,
            array(
                'subscription_order' => $this->object,
